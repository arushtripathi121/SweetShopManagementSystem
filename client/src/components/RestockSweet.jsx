...
const RestockSweet = () => {
    const { restockOpen, setRestockOpen, restockData, setSweets } = useUser();
    ...
    const handleSubmit = async () => {
        try {
            const res = await api.post(`/inventory/${restockData._id}/restock`, {
                quantity: qty
            });

            setSweets((prev) =>
                prev.map((s) => (s._id === restockData._id ? res.data.sweet : s))
            );
